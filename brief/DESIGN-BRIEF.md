...
- **Colonna 4 — Parliamone:** occhiello + una riga di invito + il **bottone CTA standard** (`.btn`, lo stesso di tutto il resto del sito — navy, radius, cornice interna, tutti gli stati) verso `modulo.html`, testo "Consulenza gratuita" senza freccia. **Storico:** partito come link testuale ottone, poi navy senza bottone ("due bottoni attaccati si annullano", la sezione 08 chiude già col suo appena sopra) — su richiesta esplicita di Simone dopo aver visto il render, diventato il bottone `.btn` pieno il 14/09/2026 (7). La freccia "→" del link testuale è stata tolta: il bottone non la usa da nessun'altra parte del sito. Classe `.f-link` e le sue regole CSS rimosse (non più usate). **Il bottone è solo su `index.html`** (14/09/2026 (4)): su `modulo.html` e `grazie.html` rimandare al modulo non ha senso (ci si è già, o la richiesta è già stata inviata) — tolto su richiesta di Simone, resta solo occhiello + invito su quelle due pagine. Il footer non è più identico byte-per-byte sulle tre pagine: unica eccezione, stesso perimetro già usato per la CTA della barra alta.
...
- **14/09/2026 (7) — il link "Parliamone" diventa bottone:** Simone ha visto il render e ha chiesto "il CTA tondo classico" al posto del link testuale ("qui metti il cta tondo classcio") — la regola precedente ("niente bottone, la sezione 08 chiude già col suo appena sopra") è stata esplicitamente ribaltata. Ora bottone `.btn` standard su `index.html`, testo "Consulenza gratuita" senza la freccia "→" (nessun'altra CTA del sito la usa). Tolte le regole `.f-link` da `sito/assets/style.css`, non più usate. §9 aggiornato.
